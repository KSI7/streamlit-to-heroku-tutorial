...
fraud=df[df.Class==1]
valid=df[df.Class==0]
outlier_fraction=(df.Class.value_counts()[1]/df.Class.value_counts()[0])
outlier_percentage = outlier_fraction*100

if st.sidebar.checkbox('Show fraud and valid transaction details'):
    st.write('Fraudulent transactions are: %.3f%%'%outlier_fraction)
...
size = st.sidebar.slider('Test Set Size', min_value=0.2, max_value=0.35)
...
clf=['Isolation Forest','Local Outlier Factor']
mod_feature = st.sidebar.selectbox('Which model for feature importance?', clf)

start_time = timeit.default_timer()
if mod_feature=='Isolation Forest':
    model=rforest
    importance=feature_sort(model,X_train,y_train)
elif mod_feature=='Local Outlier Factor':
    model=etree
    importance=feature_sort(model,X_train,y_train)
...
if st.sidebar.checkbox('Run a credit card fraud detection model'):
    ...
    elif classifier == 'Local Outlier Factor':
        model=etree
        if imb_rect=='No Rectifier':
            compute_performance(model, X_train_sfs_scaled, y_train,X_test_sfs_scaled,y_test)
        elif imb_rect=='SMOTE':
